...
pygame.display.set_caption("Campo Minado-Projeto 4")
...
#view
def infoBar():
    global gameState, start_time , time
    pygame.draw.rect(screen,BLUE,(0,0,500,100))
    pygame.draw.line(screen,BLACK,(0,100),(500,100),4)
    
    if gameState == 0: #new game
        text = font.render("Minas: " + str(game.nummines),True,YELLOW)
        text_x = text.get_rect().width
        text_y = text.get_rect().height
        screen.blit(text,((150 - (text_x / 2)),(50 - (text_y / 2))))
        text = font.render("Bandeiras: " + str(game.numflaged),True,YELLOW)
        text_x = text.get_rect().width
        text_y = text.get_rect().height
        screen.blit(text,((350 - (text_x / 2)),(50 - (text_y / 2))))
        
    	#Renderiza o tempo na tela - (Contagem não inicia adequadamente, pouco tempo pra concertar)
        #if gameState == 0:
         #      start_time = time.time()  # Inicia o cronômetro quando o jogo começa	
          #     elapsed_time = time.time() - start_time  # Calcula o tempo decorrido
           #    minutes = int(elapsed_time // 60)
            #   seconds = int(elapsed_time % 60)
             #  time_text = font.render(f"Tempo: {minutes:02}:{seconds:02}", True, YELLOW)
              # time_x = time_text.get_rect().width
               #time_y = time_text.get_rect().height
               #screen.blit(time_text, ((250 - (time_x / 2)), (80-(time_y / 2))))
        
    elif gameState == 1:      #win
        text = font.render("Parabéns,você venceu!",True,YELLOW)
        text_x = text.get_rect().width
        text_y = text.get_rect().height
        screen.blit(text,((150 - (text_x / 2)),(50 - (text_y / 2))))
    ...
    #elif gameState == 3: # record
    	#text = font.render("RECORDE",True,BLUE)
    	
    if gameState == 1 or gameState == 2:
        if button.clickButton(325,25,150,50,BLUE,WHITE,font,"Recomeçar",YELLOW):
            gameState = -1
            game.reset(0,0,0)
...
def menu():
    global gameState
    screen.fill(GREEN)
    text = font.render("CAMPO MINADO",True,BLUE)
    text_x = text.get_rect().width
    text_y = text.get_rect().height
    screen.blit(text,((250 - (text_x / 2)),(100 - (text_y / 2))))
    text = font.render("PROJETO 4",True,BLUE)
    text_x = text.get_rect().width
    text_y = text.get_rect().height
    screen.blit(text,((250 - (text_x / 2)),(150 - (text_y / 2))))
    
    if button.clickButton(200,250,100,50,BLUE,WHITE,font,"FÁCIL",YELLOW):
        game.reset(9,9,10)
        gameState = 0
    if button.clickButton(200,310,100,50,BLUE,WHITE,font,"MEDIO",YELLOW):
        game.reset(16,16,40)
        gameState = 0
    if button.clickButton(200,370,100,50,BLUE,WHITE,font,"DIFÍCIL",YELLOW):
        game.reset(30,16,99)
        gameState = 0
    if button.clickButton(200,430,100,50,BLUE,WHITE,font,"DEFINIR",YELLOW):
        gameState = -2
   # if button.clickButton(200,490,100,50,BLUE,WHITE,font,"RECORD",YELLOW):
    #    gameState = 3
        
def custom():
    global cColumns, cRows, cMines, gameState
    text = font.render("Colunas: " + str(cColumns),True,YELLOW)
    text_x = text.get_rect().width
    text_y = text.get_rect().height
    screen.blit(text,((225 - (text_x / 2)),(180 - (text_y / 2))))
    if button.clickButton(300,160,20,20,BLUE,WHITE,font," /\ ",BLACK):
        if cColumns < 20:
            cColumns += 1
    if button.clickButton(300,180,20,20,BLUE,WHITE,font," \/ ",BLACK):
        if cColumns > 0:
            cColumns -= 1
    text = font.render("Linhas: " + str(cRows),True,YELLOW)
    text_x = text.get_rect().width
    text_y = text.get_rect().height
    screen.blit(text,((230 - (text_x / 2)),(260 - (text_y / 2))))
    if button.clickButton(300,240,20,20,BLUE,WHITE,font," /\ ",BLACK):
        if cRows < 20:
            cRows += 1
    if button.clickButton(300,260,20,20,BLUE,WHITE,font," \/ ",BLACK):
        if cRows > 0:
            cRows -= 1
    text = font.render("Minas: " + str(cMines),True,YELLOW)
    text_x = text.get_rect().width
    text_y = text.get_rect().height
    screen.blit(text,((230 - (text_x / 2)),(340 - (text_y / 2))))
    if button.clickButton(300,320,20,20,BLUE,WHITE,font," /\ ",BLACK):
        if cMines < 50 and cMines < (cColumns * cRows):
            cMines += 1
    if button.clickButton(300,340,20,20,BLUE,WHITE,font," \/ ",BLACK):
        if cMines > 0:
            cMines -= 1
    if button.clickButton(200,390,100,60,BLUE,WHITE,font,"INICIAR",YELLOW):
        game.reset(cColumns,cRows,cMines)
        gameState = 0
    
    if button.clickButton(325,25,150,50,BLUE,WHITE,font,"Voltar",YELLOW):
            gameState = -1
            
#def record():  Aba funcional para implementar o recurso de record (inacabado por falta de tempo)
 #   global gameState, screen
    
  #  if button.clickButton(325,25,150,50,BLUE,WHITE,font,"Voltar",YELLOW):
		    #gameState = -1
   # text = font.render("JOGADOR",True,BLUE)
    #text_x = text.get_rect().width
    #text_y = text.get_rect().height
    #screen.blit(text,((250 - (text_x / 2)),(100 - (text_y / 2))))
...
class Tile():
    ...
    def show(self):
        ...
        pygame.draw.rect(screen,WHITE,(self.x,self.y,(size[0] / self.columns),((size[1] - 100) / self.rows)),2)
